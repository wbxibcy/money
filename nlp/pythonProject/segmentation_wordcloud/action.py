...
# 动作片
data = pd.read_csv(r'C:\Users\90586\PycharmProjects\pythonProject\整理后数据\merged_action.csv')
df = data.copy()
...
# data_count_frame.to_csv('动作片评论分词统计数据.csv',encoding='gbk')

# 语料准备
list_all = [list(jieba.cut(sentence)) for sentence in list_words]

# 将分词结果输入 word2vec 模型进行训练
model = Word2Vec(list_all, sg=0, vector_size=300, window=5, alpha=0.001, min_count=5, hs=0, negative=10, epochs=10, cbow_mean=1)

# 保存模型
model.save('word2vec_action3.model')
"""
# 生成词云图
